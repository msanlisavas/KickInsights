...
describe('KI_EstimationEngine', () => {
  describe('estimate', () => {
    test('calculates estimated viewers from unique chatters and participation rate', () => {
      const result = KI_EstimationEngine.estimate(100, 0.05);
      expect(result.estimatedViewers).toBe(2000);
    });

    test('returns 0 when no chatters', () => {
      const result = KI_EstimationEngine.estimate(0, 0.05);
      expect(result.estimatedViewers).toBe(0);
    });

    test('handles small participation rate', () => {
      const result = KI_EstimationEngine.estimate(50, 0.03);
      expect(result.estimatedViewers).toBeCloseTo(1667, 0);
    });
  });

  describe('confidence', () => {
    test('returns "low" for fewer than 20 unique chatters', () => {
      const result = KI_EstimationEngine.estimate(10, 0.05);
      expect(result.confidence).toBe('low');
    });

    test('returns "medium" for 20-100 unique chatters', () => {
      const result = KI_EstimationEngine.estimate(50, 0.05);
      expect(result.confidence).toBe('medium');
    });

    test('returns "high" for more than 100 unique chatters', () => {
      const result = KI_EstimationEngine.estimate(150, 0.05);
      expect(result.confidence).toBe('high');
    });

    test('returns "medium" at exactly 20 chatters', () => {
      const result = KI_EstimationEngine.estimate(20, 0.05);
      expect(result.confidence).toBe('medium');
    });

    test('returns "high" at exactly 101 chatters', () => {
      const result = KI_EstimationEngine.estimate(101, 0.05);
      expect(result.confidence).toBe('high');
    });
  });
});
